...
export const GetExerciseResult = (series: Array<number>, target:number):IResultExercise => {
  
  const result = {periodLength: 0,
    trainingDays: 0,
    success: false,
    rating: 0,
    ratingDescription: '',
  ...
  result.periodLength = series.length;
  series.forEach(n => n > 0 ? result.trainingDays++ : null);
  result.average = series.reduce((a, b) => a + b) / result.periodLength;
  result.success = result.average > 1 && result.trainingDays > result.periodLength * 0.7;
  result.average < 1 ? result.rating = 1 : result.rating = 2;
  result.average > 2 ? result.rating = 3 : null; 
  switch (result.rating) {
    case 1:        
      result.ratingDescription = 'looser';
      break;
    ...
    case 3:
      result.ratingDescription = 'outstanding';
      break;
