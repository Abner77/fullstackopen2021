...
export const createAnecdote = ( desc ) => {  
 return async dispatch => {   
    const anCreated = await anecdotesServices.create(asObject(desc))   
    dispatch({type: 'CREATE', data: anCreated.data })
  } 
}

export const upvoteAnecdote = ( anecdote ) => {
  return async dispatch => {
    const anresult = await anecdotesServices.update(anecdote)
    console.log('resultado de update', anresult.data)
    dispatch ({ type: 'UPVOTE', data: anresult.data})
  }  
}

export const initAnecdotes = () => {
  return async dispatch => {
    const anresult = await anecdotesServices.getAll()    
    dispatch({
      type: 'INIT',
      data: anresult.data
    })
